...
function [thick,area_res,perm,por,dens_c,visc_c,visc_w,compr,rc,...
    gamma,delta,omega,depth_seismic,depth_water,pres_grad_seismic,Sv_grad_seismic,...
    Shmin_grad_seismic,SHmax_grad_seismic,SHmax_dir_seismic,fault_friction,...
    pres_offset,Sv_offset,Shmin_offset,SHmax_offset] = read_data(path,name)
    ...
    % -- read data
    data = readtable([path,'\',name],'Sheet','Model parameters','Range','A1:AI2'); 
    ...
    depth_water = double(data{1,5});                    % mean sea depth [m]
    thick = double(data{1,6});                          % thickness of reservoir [m]
    area_res = double(data{1,7});                       % area of reservoir [km^2]
    perm = double(data{1,8})*10^-15;                    % intrinsic permeability [m^2]
    por = double(data{1,9});                            % porosity [-]
    cr = double(data{1,10})/1e6;                        % rock compressibility [1/Pa]
    cw = double(data{1,11})/1e6;                        % water compressibility [1/Pa]
    dens_c = double(data{1,12})*1e3;                    % Density of CO2 [kg/m^3]
    visc_c = double(data{1,13})/1e3;                    % Viscosity of CO2 [Pa.s] 
    visc_w = double(data{1,14})/1e3;                    % Viscosity of water[Pa.s] 
    pres_grad = double(data{1,15})/1e3;                 % pressure gradient [MPa/m]
    pres_offset = double(data{1,16});                   % pressure offset [MPa]
    temp_surf = double(data{1,17});                     % surface temperature [C]
    temp_grad = double(data{1,18})/1e3;                 % temperature gradient [C/m]
    Sv_grad = double(data{1,19})/1e3;                   % overburden stress offset [MPa]
    Sv_offset = double(data{1,20});                     % overburden stress gradient [MPa/m]
    Shmin_grad = double(data{1,21})/1e3;                % Shmin stress gradient [MPa/m]
    Shmin_offset = double(data{1,22});                  % Shmin stress offset [MPa]
    SHmax_grad = double(data{1,23})/1e3;                % SHmax stress gradient [MPa/m]
    SHmax_offset = double(data{1,24});                  % SHmax stress offset [MPa]
    salinity = double(data{1,25})/1e6;                  % aquifer salinity [ppm/1e6]
    rock_friction = double(data{1,26});                 % rock friction angle  [deg]
    rock_cohesion = double(data{1,27});                 % rock cohesion coefficient [MPa]
    rock_tens_strength = double(data{1,28});            % rock tensile strength [MPa]

    % Seismicity data
    depth_seismic = double(data{1,29});                 % depth at which seismicity may occur [m]
    pres_grad_seismic = double(data{1,30})/1e3;         % pore pressure gradient (MPa/m)
    Sv_grad_seismic = double(data{1,31})/1e3;           % overburden stress gradient (MPa/m)
    Shmin_grad_seismic = double(data{1,32})/1e3;        % Shmin stress gradient (MPa/m)
    SHmax_grad_seismic = double(data{1,33})/1e3;        % SHmax stress gradient (MPa/m)
    SHmax_dir_seismic = double(data{1,34});             % SHmax stress direction (deg)
    fault_friction = double(data{1,35});                % fault friction coefficient
    % --
    ...
    pres_mean = pres_offset + pres_grad*(depth_mean - depth_water) ;     % pressure at mid reservoir depth [MPa]
    T_mean = temp_surf + temp_grad*(depth_mean - depth_water);           % temperature at mid reservoir depth [C]
