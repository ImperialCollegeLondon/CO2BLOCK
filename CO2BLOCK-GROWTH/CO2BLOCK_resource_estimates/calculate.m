function [site_name,d_list,well_list,d_max,Q_M_each,V_M,Table_Q,Table_V,p_sup_vec]...
    ...
    if isfinite(rc) && rc<R_influence
        ...
        n_lookup_r = 80;       % radial positions for source well
        n_quad_r   = 70;       % radial integration points
        n_quad_th  = 120;      % angular integration points
    ...
    for x_grid_num = 1:sqrt(nr_well_max)                                    % number of wells on a horizontal row
        ...
        for y_grid_num = x_grid_num:x_grid_num+plus                         % number of wells on a  vertical row
            ...
            for d = 1:nr_dist
                ...
                nbs = arrayfun(@(r) nordbotten(r,R_influence,psi,rc,gamma), dist_vec,"UniformOutput",true);
                nbs_inf = arrayfun(@(r) nordbotten(r,R_influence,psi,inf,gamma), dist_vec,"UniformOutput",true);
