...
> **Rule**: If a region is not an Evenick (2021) basin, it should not be added to Global.xlsx as a separate basin entry. CCS-relevant context for non-basin regions should be tracked in a companion CCS-readiness layer rather than inflating the basin classification table.
...
> **Practical impact**: Zero CCS projects exist in the St. Lawrence Lowlands. The screening gap is limited to Quebec-specific storage capacity estimates (2.8–3.2 Gt prospective) and regulatory context (no CCS Act, no pore-space tenure).
