...
% rate change
rtarget4z = [0.132606231984019]; % Net negative 4.7 Gt
% resulting storage to meet 2100 target

Qtarget4z =[38.8375742985741]; % mod 506, 366, 50.6, 36.6 - obs 500.2358, 360.2792, 50.8073, 36.0902
% resulting peak injection
peak_target4z = [2056.60330165083];
...
for i=1:length(Qtarget4z)
    ...
    C = (Qtarget4z(i)-cum_at_rate_change);
    pt = (C./(1+exp(rtarget4z(i)*(peak_target4z(i)-x2))));
...
legend({[num2str(rtarget4z(1)*100), '%, supported by  500 Gt']}, ...
...
for i=1:length(Qtarget4z)
    ...
    yrate2 = (C.*rtarget4z(i).*exp(rtarget4z(i).*(peak_target4z(i)-x2)))./((1+exp(rtarget4z(i).*(peak_target4z(i)-x2))).^2);
...
inflection_time_red = peak_target4z-log(2+sqrt(3))./rtarget4z;
C = (Qtarget4z-cum_2030);
y_inflect_red = (C.*rtarget4z.*exp(rtarget4z.*(peak_target4z-inflection_time_red)))./...
        ((1+exp(rtarget4z.*(peak_target4z-inflection_time_red))).^2);
