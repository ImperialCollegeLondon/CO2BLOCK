% This version is to calculate storage capacity (primarily for the UK) for
...
fname = 'Australia_sites.xlsx';        % name of the input data file

allocation_order = "descend";                    % ascend: allocating resources in the order of ascending storage capacity (smallest first)
...
if storage_resource_calculation == 'savedfile'
    ...
    load('Resources_Australia_150y.mat');

    % growth rate model for the target 175 Mt/y on 2050 and 100 years injection
    load('Growth_Australia_150y.mat')
    Growthrate_model = Growth_Australia_150y;

    inj_duration = 150;
    
    allocation_duration = 150;
    ...
    for time_yr = 10:10:inj_duration
    
        %-- setting parameters
        nr_region = 11;                     % number of storage regions in the input file
    ...
    writetable(Site_no_all_scenarios_table,['Site_no_summary_minQ=',num2str(minQ),'_maxQ=',num2str(maxQ),'_sites=',num2str(nr_well_max),'_',num2str(time_yr),'y','.xlsx']);
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%          Allocation of storage rate to subsurface resources


years = Growthrate_model(1:find(Growthrate_model == Growthrate_model(1,1)+allocation_duration-1),1);
total_rate = Growthrate_model(1:find(Growthrate_model == Growthrate_model(1,1)+allocation_duration-1),2);
...
elseif allocation_order == 'ascend'
    [~, sort_idx] = sort(sum(resource_rate_matrix,2));
elseif allocation_order == 'random'
    ...
    for i=idx:-1:1
        ...
        if (vol_possible(i) > area_growthrate || abs(vol_possible(i)-area_growthrate)<0.01) && resource_rate_matrix_sorted(allocation_step,i) >= storage_resource_step(allocation_step) 
            ...
            if storage_resource_total_provisional >= max(total_rate)
                ...
                [region_name,d_list,well_list,d_max,Q_M_each,V_M,Table_Q,Table_V,p_sup_vec] = calculate(fpath,fname,sort_idx(allocation_step),correction,dist_min,...
                     dist_max,nr_dist,nr_well_max,rw,storage_periods(i),maxQ,minQ);
...
writetable(Resource_assignment_table,['Resource_assignment_minQ=',num2str(minQ),'_maxQ=',num2str(maxQ),'_sites=',num2str(nr_well_max),'_',num2str(time_yr),'y','.xlsx']);
...
% text(2013, 2000, '(b)', 'FontSize',10, 'Interpreter','tex')
...
disp(['Total number of sites used = ',num2str(sum(site_no_step))])

% Export as vector PDF for journal submission
exportgraphics(gcf, ['storage_assignment_minQ=',num2str(minQ),'_maxQ=',num2str(maxQ),'_sites=',num2str(nr_well_max),'_',num2str(time_yr),'y','.pdf'], 'ContentType','vector');
exportgraphics(gcf, ['storage_assignment_minQ=',num2str(minQ),'_maxQ=',num2str(maxQ),'_sites=',num2str(nr_well_max),'_',num2str(time_yr),'y','.png'], ...
    'Resolution', 600, 'BackgroundColor', 'white');
saveas(gcf, ['storage_assignment_minQ=',num2str(minQ),'_maxQ=',num2str(maxQ),'_sites=',num2str(nr_well_max),'_',num2str(time_yr),'y','.svg']);
...
ylabel('Number of sites','FontSize',8,'FontName','Helvetica')

ylim([800 2400])
hold on

% Limited horizontal segments (LEFT axis)
plot([0 25],[1632 1632],'k--','LineWidth',0.8,'HandleVisibility','off')
plot([0 25],[1235 1235],'k--','LineWidth',0.8,'HandleVisibility','off')
plot([0 25],[1324 1324],'k--','LineWidth',0.8,'HandleVisibility','off')

% LEFT axis annotations
text(2,1632,'prioritising large units', ...
    'FontSize',7,'FontName','Helvetica', ...
    'VerticalAlignment','bottom')

text(2,1235,'prioritising small units', ...
    'FontSize',7,'FontName','Helvetica', ...
    'VerticalAlignment','top')

text(2,1324,'planned projects path', ...
    'FontSize',7,'FontName','Helvetica', ...
    'VerticalAlignment','bottom')
...
ylabel('Number of regions','FontSize',8,'FontName','Helvetica')

ylim([-20 30])
hold on

% Limited horizontal segments (RIGHT axis)
plot([75 100],[7 7],'k--','LineWidth',0.8,'HandleVisibility','off')
...
text(82,7,'prioritising large units', ...
    'FontSize',7,'FontName','Helvetica', ...
    'VerticalAlignment','bottom')

text(82,27,'prioritising small units', ...
    'FontSize',7,'FontName','Helvetica', ...
    'VerticalAlignment','top')
...
exportgraphics(gcf, ['random allocation','.pdf'], 'ContentType','vector');
exportgraphics(gcf, ['random allocation','.png'], ...
    'Resolution', 600, 'BackgroundColor', 'white');
saveas(gcf, ['random allocation','.svg']);
