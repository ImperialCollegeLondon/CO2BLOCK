...
> **Note:** The three basins listed below are high-profile examples relevant to CCS, but they are not exhaustive. A complete inventory of all **34 Canadian basins** absent from `Global.xlsx` (the vast majority with zero CCS activity) is documented in `CANADA-CCS-REMAINING_BASINS_ZERO.md`.

| Missing Basin | Relevance |
|---|---|
| **Williston Basin** | Hosts **Boundary Dam CCS** (Saskatchewan, operational 2014) — Canada's other major CCS project. Boundary Dam is a Tier 1 CCS project often paired with Alberta projects in national CCS accounting. *See also: `CANADA-CCS-WILLISTON-missing.md`.* |
| **Sverdrup Basin** | Canadian Arctic Islands — potential future storage, mentioned in NRCan/GSC Arctic assessments (no dedicated CCS study). *See also: `CANADA-SVERSDRUP-CCS.md`.* |
...
- Cross-referenced against `evenick2021.csv` (Evenick 2021 global basin classification) — Alberta Basin UBI 13 confirmed present in the source dataset (Basin Name: "Alberta", UBI: 13, Countries: Canada/USA, Type: Foreland, Setting: Onshore, Area: 1,602,675 km²). This confirms the omission is a **Global.xlsx pipeline loss**, not a basin-classification error.
- No variation of "Alberta", "WCSB", "Western Canadian Sedimentary Basin", "Williston", or "Saskatchewan" found in any cell of Global.xlsx
